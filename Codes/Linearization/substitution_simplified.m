...
hValues = [380 ...
    ...
    280 525 ...
    (525*DmValues(4)+460*DmValues(2))/DmValues(1)]*10^3; % h1 hL hG hR hMT hHR
hBPValue = 298*10^3;
...
wValue = dValues(5)*DVValues(1)*sigmaValues(3)/(sigmaValues(1)+sigmaValues(2)*DVValues(1));
T1Value = CoolProp.PropsSI('T','P',pValues(1),'H',hValues(1),'CO2');
TA0Values = 30+273.15;
TA1Value = 1/(wValue+1)*T1Value+wValue/(wValue+1)*TA0Values;
TValues = [TA1Value T1Value TA0Values]; % TA1 T1 TA0
...
for it = 1:numel(fields)
    ...
    mx4sub.(fields{it}) = subs(mx4sub.(fields{it}),{h1 hL hG hR hMT hHR},num2cell(hValues));
